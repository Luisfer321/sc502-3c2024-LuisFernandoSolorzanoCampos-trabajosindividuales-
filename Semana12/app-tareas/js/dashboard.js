...
async function cargarComentarios(taskId) {
    const res = await fetch(`http://localhost:8000/comments.php?task_id=${taskId}`);
    const comentarios = await res.json();
    console.log("Comentarios:", comentarios);
    // Acá podrías mostrarlos dentro del modal si querés
}

// Agregar comentario a una tarea
async function guardarComentario(taskId, comentario) {
    const res = await fetch('http://localhost:8000/comments.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    console.log(data);
    alert(data.message || "Comentario guardado");
    // recargar comentarios o tareas
}